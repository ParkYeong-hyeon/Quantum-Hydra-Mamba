...
# Paths (from config)
OUTPUT_DIR = get_output_dir('eeg')
JOBS_DIR = get_jobs_dir('eeg')

# SLURM Configuration (from config, can be overridden)
slurm_config = get_slurm_config(time='48:00:00')  # EEG needs longer time
...
#SBATCH --output={str(OUTPUT_DIR)}/logs/eeg_{model_name}_seed{seed}.log
#SBATCH --error={str(OUTPUT_DIR)}/logs/eeg_{model_name}_seed{seed}.log
...
# Run training
$PYTHON_CMD {str(get_script_path('run_single_model_eeg.py'))} \\
...
"""
    ...
    for i, job_file in enumerate(job_files, 1):
        ...
        master_script += f"sbatch job_scripts/eeg/{job_file}\n"
...
"""
    ...
    print(f"   bash job_scripts/eeg/eeg_quantum_hydra_seed2024.sh")
    print()
    print("3. Monitor progress:")
    print(f"   tail -f experiments/eeg_results/logs/*.log")
    print()
    print("4. After completion, aggregate results:")
    print(f"   python scripts/aggregation/aggregate_eeg_results.py")
    print("="*80)
